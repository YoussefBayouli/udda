...
class SchemaObjectFactory {
    ...
    mergePropertyWithMetadata(key, prototype, schemas, pendingSchemaRefs, metadata) {
        ...
        if (this.isObjectLiteral(metadata.type)) {
            return this.createFromObjectLiteral(key, metadata.type, schemas);
        }
        ...
        if (!is_built_in_type_util_1.isBuiltInType(metadata.type)) {
            return this.createNotBuiltInTypeReference(key, metadata, schemas, pendingSchemaRefs);
        }
    ...
    createNotBuiltInTypeReference(key, metadata, schemas, pendingSchemaRefs) {
        ...
        if (!(schemaObjectName in schemas) &&
            ...
            schemaObjectName = this.exploreModelSchema(metadata.type, schemas, [...pendingSchemaRefs, schemaObjectName]);
        }
    ...
    transformToArraySchemaProperty(metadata, key, type) {
        ...
        const movedProperties = lodash_1.pick(metadata, keysToMove);
