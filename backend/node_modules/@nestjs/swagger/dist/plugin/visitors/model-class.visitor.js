...
class ModelClassVisitor extends abstract_visitor_1.AbstractFileVisitor {
    ...
    createDecoratorObjectLiteralExpr(node, typeChecker, existingProperties = ts.createNodeArray(), options = {}, hostFilename = '', sourceFile) {
        ...
        const isNullable = !!node.questionToken || ast_utils_1.isNull(type) || ast_utils_1.isUndefined(type);
        let properties = [
            ...existingProperties,
            ...
            !plugin_utils_1.hasPropertyKey('nullable', existingProperties) && isNullable &&
                ts.createPropertyAssignment('nullable', ts.createLiteral(isNullable)),
            this.createTypePropertyAssignment(node.type, typeChecker, existingProperties, hostFilename),
    ...
    createTypePropertyAssignment(node, typeChecker, existingProperties, hostFilename) {
        ...
        if (node) {
            if (ts.isTypeLiteralNode(node)) {
                ...
                const nullableType = ast_utils_1.findNullableTypeFromUnion(node, typeChecker);
